...
@if(count($slides) > 0)
    <section class="container pt-4 mb-4 pb-sm-2 pb-md-3 pb-lg-4 pb-xl-5">
                        ...
                        <div class="swiper-wrapper">
                            @foreach($slides as $slide)
                                <div class="swiper-slide text-center text-xl-start pt-5 py-xl-5">
                                    @if($slide->subtitle)
                                        <p class="text-body">{{ $slide->subtitle }}</p>
                                    @endif
                                    <h2 class="display-4 pb-2 pb-xl-4">{{ $slide->title }}</h2>
                                    @if($slide->link && $slide->link_text)
                                        <a class="btn btn-lg btn-primary" href="{{ $slide->link }}">
                                            {{ $slide->link_text }}
